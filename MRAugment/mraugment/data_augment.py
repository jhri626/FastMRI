...
class AugmentationPipeline:
    ...
    each augmentation.
    """
    def __init__(self, hparams, seed=None): # 0713 seed fix추가
    ...
    def augment_from_kspace(self, kspace, target_size, max_train_size=None): 
        ...
        #print(f"Augmented kspace shape: {kspace.shape}")
        #print(f"Augmented target shape: {target.shape}")
...
class DataAugmentor:
    ...
    def __init__(self, hparams, current_epoch_fn, seed=None): # 0713 seed fix추가
        """
        ...
        if self.aug_on:
            self.augmentation_pipeline = AugmentationPipeline(hparams,seed) # 0713 seed fix추가
    ...
    def __call__(self, kspace, target_size):
        ...
        if self.aug_on and p > 0.0:
            #print(f"Data augmentation applied with probability {p:.2f}")
